const validePassword = (password, register) => {
    ...
    const regex = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[._\-,@])[A-Za-z\d._\-@]{8,40}$/

    if (!register || regex.test(password)) {
        return { "ok": true, "message": "contraseña validada correctamente" }
    } else {
        return { "ok": false, "message": "la contraseña no cumple con el formato, debe de  entre 8 y 40 caracteres, 1 número y al menos un simbolo entre (. _ - @)" }
    }
}
